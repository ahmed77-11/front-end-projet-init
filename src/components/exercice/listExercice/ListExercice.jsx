import "./listExercice.css";
import { useEffect } from "react";
import { useDispatch, useSelector } from "react-redux";
import {deleteExercice, getAllExercices} from "../../../redux/exercice/exerciceSlice.js";
import {useNavigate} from "react-router-dom";
 ...
 const ListExercice = () => {
    ...
    const handleEdit = (id) => {
        ...
        window.location.href = `/updateExercice/${id}`;
    };
                ...
                <table className="profile-table">
                    ...
                    {exercices.length > 0 ? (
                        exercices.map((exercice) => (
                            <tr key={exercice.id}  style={{cursor:"pointer"}} onClick={()=>{navigate("/admin/dashboard/detailExercice/"+exercice.id)}}>
                                <td>{exercice.id}</td>
                                <td>{exercice.nom}</td>
